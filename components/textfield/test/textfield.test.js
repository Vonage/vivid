...
describe('textfield', () => {
	...
	describe(`form association`, function () {
		...
		describe(`submit form on Enter key`, function () {
			...
			const fieldNameSE = 'test-field';
			const fieldValueSE = Math.random().toString();
			...
			beforeEach(async function () {
				[formElement, externalForm] = addElement(
					textToDomToParent(`
				<form onsubmit="return false" name="testForm" id="testForm">
					<${COMPONENT_NAME} name="${fieldNameSE}" value="${fieldValueSE}" form="${externalFormID}">
					</${COMPONENT_NAME}>
				</form>
			...
			it(`should submit form on enter key press with button without a type`, async function () {
				...
				for (let [formDataKey, formDataValue] of (await submitPromise).entries()) {
					expect(formDataKey).to.equal(fieldNameSE);
					expect(formDataValue).to.equal(fieldValueSE);
				}
			});
