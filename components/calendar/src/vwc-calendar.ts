...
export class VWCCalendar extends LitElement {
	...
	#daysLength = 7;
	#hoursOfDayDisplayed = (Array.from({ length: 23 }) as Date[])
	...
	protected renderTimeCells(): TemplateResult[] {
		const templates = [];
		for (let i = 0; i < (this.#hoursOfDayDisplayed.length + 1) * this.#daysLength; i++) {
	...
	protected renderHours(): TemplateResult {
		return html`
			<ol class="time">
				...
				${this.#hoursOfDayDisplayed.map(h => html`<li>
