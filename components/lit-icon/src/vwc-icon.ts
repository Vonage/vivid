import { LitElement, html, property, customElement, CSSResult, TemplateResult } from 'lit-element';
...
const
	BASE_URL = 'https://icons.resources.vonage.com/3f7739a0-a898-4f69-a82b-ad9d743170b6',
...
const baseUrlTemplate = (resource:string, version?:string)=> [BASE_URL, version && `v${ICON_SET_VERSION}`, resource].filter(Boolean).join('/');
const resolveIcon = memoizeWith(
	identity as ()=> string,
	(iconId = '')=>
		iconId.trim()
			? fetch(baseUrlTemplate([iconId, 'svg'].join('.'), ICON_SET_VERSION))
				.then((res)=> res.headers.has('x-amz-website-redirect-location')
					? fetch(baseUrlTemplate((res.headers.get('x-amz-website-redirect-location') ?? '').slice(1)))
					: res
				)
				.then((res)=> res.headers.get('content-type') === 'image/svg+xml'
					? res.text()
					: ''
				)
				.then(unsafeSVG)
