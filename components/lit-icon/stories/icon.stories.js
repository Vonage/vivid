...
const registerCategory = (categoryTitle, content) => storiesOf(['Components', 'Atoms', 'Lit-Icon', 'Types'].join(PATH_SEPARATOR), module)
	.add(categoryTitle, () => {
...
_(icons)
	...
	.forEach((list, category)=> {
			registerCategory(
				...
				list.map(({ id: icon_id }) => `<figure><vwc-lit-icon title=${icon_id} size="large" type="${icon_id}"></vwc-lit-icon><figcaption>${icon_id}</figcaption></figure>`).join('\n')
			);
	});