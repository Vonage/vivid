...
export { relocateStaticResources };

const CONFIGS_LOOKUP_PATTERN =
	'+(common|components|docs)/**/stories/**/*.config.+(js|mjs)';
...
async function build() {
	...
	console.log(
		`... transformation of MDs to stories DONE (${Math.round(
...
async function processJsConfiguration(configPath) {
	...
	let mdText = fs.readFileSync(
		resolve(configDir, config.sourcePath),
		FS_OPTIONS
	);
	...
	const outputFileName =
		config.outputName +
		(config.outputName.endsWith('.js') ? '' : DEFAULT_OUTPUT_FILE_SUFFIX);
...
function validateConfig(config) {
	...
	if (!config.sourcePath || typeof config.sourcePath !== 'string') {
		throw new Error(
			`'sourcePath' MUST be a non-empty string; got '${config.sourcePath}'`
		);
	}
	if (!config.outputName || typeof config.outputName !== 'string') {
		throw new Error(
			`'outputPath' MUST be a non-empty string; got '${config.outputName}'`
		);
	}
	...
	if (!config.story.title || typeof config.story.title !== 'string') {
		throw new Error(
			`'story.title' MUST be a non-empty string; got '${config.story.title}'`
		);
	}
	if (!config.story.name || typeof config.story.name !== 'string') {
		throw new Error(
			`'story.name' MUST be a non-empty string; got '${config.story.name}'`
		);
	}

	if ('mdPreProcess' in config && typeof config.mdPreProcess !== 'function') {
		throw new Error(
			`'mdPreProcess', when provided, MUST be a function; got '${config.mdPreProcess}'`
		);
	}
	if (
		'htmlPostProcess' in config &&
		typeof config.htmlPostProcess !== 'function'
	) {
		throw new Error(
			`'htmlPostProcess', when provided, MUST be a function; got '${config.htmlPostProcess}'`
		);
	}
}
...
const AUTOGENERATED_FOLDER_RELATIVE = 'assets/autogenerated',
...
function relocateStaticResources(htmlInput, srPaths, srBasePath = '') {
	...
	for (const srPath of srPaths) {
		const autogeneratedFileName = srPath.match(/\/(?<fileName>[^/]+)$/).groups
			.fileName;
		const effectiveSrBasePath = srBasePath ? `${srBasePath}/` : '';
		fs.copyFileSync(
			`${effectiveSrBasePath}${srPath}`,
			`${AUTOGENERATED_FOLDER_ABSOLUTE}/${autogeneratedFileName}`
		);
		if (htmlOutput) {
			htmlOutput = htmlOutput.replace(
				new RegExp(`(href|src)\s*=\s*"?${srPath}"?`, 'g'),
				`$1="${AUTOGENERATED_FOLDER_RELATIVE}/${autogeneratedFileName}"`
			);
		}
	}
	return htmlOutput;
